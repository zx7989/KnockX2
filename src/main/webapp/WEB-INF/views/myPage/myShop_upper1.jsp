...
<body>
  ...
  <div class="shopTitle01">
    <c:choose>
      <c:when test="${member.imageFN == null}">
        <img  src="${contextPath}/resources/image/profile.png" id="preview"  />
        
      </c:when>
      <c:when test="${member.imageFN != null}">
        <img src="${contextPath}/download1.do?id=${member.id}&imageFN=${member.imageFN}" id="preview"  />
      </c:when>
    </c:choose>
    <h1 class="shopName1">My Shop
...
</h1><h1 class="title02 title03"><span style="font-size: 60px;">${shop.shop_name}</span><span>welcome</span></h1>
